...
BOOST_AUTO_TEST_SUITE(Base32CrockfordTestCase)
...
SimpleBaseLibStringArray EncodedData = SimpleBaseLibStringArray({ "", "CR", "CSQG", "CSQPY",
	"CSQPYRG", "CSQPYRK1", "CSQPYRK1E8",
	"64S36D1N6RVKGE9G64S36D1N6RVKGE9G64S36D1N6RVKGE9G64S36D1N6RVKGE9G"
});

SimpleBaseLibStringArray SpecialRaw = SimpleBaseLibStringArray({ "000", "111", "111" });
SimpleBaseLibStringArray SpecialEncoded = SimpleBaseLibStringArray({ "O0o", "Ll1", "I1i" });

IBase32 base32 = SimpleBaseFactory::CreateBase32Crockford();

BOOST_AUTO_TEST_CASE(Test_Decode_CrockfordChars_DecodedCorrectly)
{
	SimpleBaseLibByteArray result, expectedResult;

	for (register size_t i = 0; i < SpecialRaw.size(); i++)
	{
		expectedResult = base32->Decode(SpecialRaw[i]);
		result = base32->Decode(SpecialEncoded[i]);
		BOOST_CHECK(expectedResult == result, "Decoding Failed at Index %d', [i]");
	}
}

BOOST_AUTO_TEST_CASE(Test_Decode_InvalidInput_ThrowsInvalidArgumentException)
{
	BOOST_CHECK_THROW(base32->Decode("[];',m."), InvalidArgumentSimpleBaseLibException);
}
